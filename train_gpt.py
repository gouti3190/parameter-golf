...
def apply_rotary_emb(
    ...
    if rope_dims > x.size(-1):
        raise ValueError(
            f"rope_dims={rope_dims} cannot exceed head_dim={x.size(-1)}"
        )
    ...
    if cos.size(-1) != half or sin.size(-1) != half:
        raise ValueError(
            f"RoPE cache mismatch: cos/sin last dim should be {half}, "
            f"but got cos={cos.shape}, sin={sin.shape}, rope_dims={rope_dims}"
        )
...
class CausalSelfAttention(nn.Module):
    def __init__(
        ...
        if rope_dims > self.head_dim:
            raise ValueError(
                f"rope_dims={rope_dims} cannot exceed head_dim={self.head_dim}"
            )
        ...
        # IMPORTANT FIX:
        # Rotary cache must be built for rope_dims, not full head_dim.
        rotary_dim = rope_dims if rope_dims is not None else self.head_dim
...
class Block(nn.Module):
    def __init__(
        ...
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init, rope_dims=16)
